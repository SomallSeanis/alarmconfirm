...
public class AlarmSqlCacheConstants {
    ...
    private static final String SQL_2 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 2 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_3 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 3 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_4 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 4 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_5 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 5 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_6 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 6 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_7 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 7 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_8 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 8 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_9 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 9 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_10 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 10 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_11 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 11 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_12 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 12 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_13 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 13 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_14 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 14 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_15 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 15 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_16 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 16 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_17 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 17 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_18 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 18 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_19 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 19 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_20 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 20 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_21 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 21 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_22 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 22 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_23 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 23 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_24 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 24 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_25 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 25 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_26 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 26 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_27 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 27 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
    private static final String SQL_28 = "select A.StationId, A.PointId, A.AlarmRuleId, C.[Order] as AlarmOrder, A.AlarmType, B.MaxTime from AlarmRealTimeInfoes as A INNER JOIN (select StationId, PointId,max(AlarmDateTime) as MaxTime from AlarmRealTimeInfoes WHERE AlarmStatus =0 and StationId = 28 group by StationId,PointId)B" +
            " on A.StationId=B.StationId AND A.PointId=B.PointId AND  A.AlarmDateTime=B.MaxTime LEFT JOIN AlarmLevels as C on C.EntityId = A.AlarmLevel_EntityId;";
